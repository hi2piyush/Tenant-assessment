...
##          Please fully read and test any scripts before running in your production environment!
        ...
        Required - Thumbprint of the certificate generated from the prepare-tenantassessment.ps1 script or from the steps to manually register the app.
...
$ProgressTracker = 1
$TotalProgressTasks = 29 #####Updated from 27 to 29######
$ProgressStatus = $null
...
$apiuri = "https://graph.microsoft.com/v1.0/users?`$select=id,userprincipalname,mail,displayname,givenname,surname,jobTitle,department,licenseAssignmentStates,proxyaddresses,usagelocation,onPremisesImmutableId,usertype,accountenabled"
...
[array]$SoftDeletedMailboxes = Get-Mailbox -SoftDeletedMailbox -ResultSize Unlimited

[array]$LitigationHoldMailboxes =  Get-Mailbox -ResultSize Unlimited | where-object {$_.LitigationHoldEnabled -eq $true}

[array]$SendOnBehalfOf = Get-Mailbox -ResultSize Unlimited | where {$_.GrantSendOnBehalfTo -ne $null}

[array]$PublicFolder = Get-PublicFolder -Recurse -ResultSize Unlimited
[array]$PublicFolderClientPermission = Get-PublicFolder -Recurse -ResultSize Unlimited | Get-PublicFolderClientPermission

#############################################

$ProgressStatus = "Getting Soft Deleted Mailbox statistics..."
UpdateProgress
$ProgressTracker++

$i = 1

##Get Soft Deleted Mailbox Sizes
foreach ($soft in $SoftDeletedMailboxes) {
    $ProgressStatus = "Getting Soft Deleted Mailboxes statistics $i of $($SoftDeletedMailboxes.count)..."
    $i++
    UpdateProgress

    $SoftStats = $null
    $SoftStats = get-EXOmailboxstatistics -IncludeSoftDeletedRecipients $soft.primarysmtpaddress
    $soft | Add-Member -MemberType NoteProperty -Name MailboxSize -Value $softStats.TotalItemSize -Force
    $soft | Add-Member -MemberType NoteProperty -Name ItemCount -Value $softStats.ItemCount -Force

    ##Clean email addresses value
    $soft.EmailAddresses = $soft.EmailAddresses -join ';'
}
...
Try {
    ...
    $users | ? { ($_.usertype -ne "Guest") -and ($_.mailboxtype -eq "User") } | Select-Object Migrate, id, accountenabled, userPrincipalName, mail, targetobjectID, targetUPN, TargetMail, displayName, MailboxItemCount, MailboxSizeGB, OneDriveSizeGB, OneDriveFileCount, MailboxType, ArchiveSizeGB, ArchiveItemCount, givenName, surname, jobTitle, department, proxyaddresses, 'License SKUs', 'Group License Assignments', 'Disabled Plan IDs', onPremisesImmutableId, usagelocation, usertype | Export-Excel -Path ("$FilePath\$Filename") -WorksheetName "User Accounts" -AutoSize -AutoFilter -FreezeTopRow -BoldTopRow 
...
#######################################
##Export Soft Deleted Mailbox tab
$SoftDeletedMailboxes | Select-Object Migrate,Name,DisplayName,UserPrincipalName,WindowsLiveID,EmailAddresses,PrimarySmtpAddress,Alias,Identity,LegacyExchangeDN,ItemCount,MailboxSize,MailboxRegion,ExchangeGuid,LitigationHoldEnabled,LitigationHoldDate,LitigationHoldOwner,LitigationHoldDuration,WasInactiveMailbox,WhenMailboxCreated,WhenSoftDeleted,RecipientTypeDetails,ExternalDirectoryObjectId,ImmutableId,SamAccountName| Export-Excel -Path ("$FilePath\$Filename") -WorksheetName "SoftDeletedMailboxes" -AutoSize -AutoFilter -FreezeTopRow -BoldTopRow

##Export Litigation Hold Mailboxes tab
$LitigationHoldMailboxes |Select-Object Name,DisplayName,PrimarySmtpAddress,LitigationHoldDate,LitigationHoldOwner,LitigationHoldDuration | Export-Excel -Path ("$FilePath\$Filename") -WorksheetName "LitigationHoldMailboxes" -AutoSize -AutoFilter -FreezeTopRow -BoldTopRow

##Export SendOnBehalfOf tab
$SendOnBehalfOf |Select-Object Name,Alias,UserPrincipalName,PrimarySmtpAddress,@{l='SendOnBehalfOf';e={$_.GrantSendOnBehalfTo -join ";"}} | Export-Excel -Path ("$FilePath\$Filename") -WorksheetName "SendOnBehalfOf" -AutoSize -AutoFilter -FreezeTopRow -BoldTopRow

##Export Public Folder tab
$PublicFolder |Select-Object Identity,Name,MailEnabled,ParentPath,ContentMailboxName,ContentMailboxGuid,FolderSize,HasSubfolders,FolderClass,@{l='FolderPath';e={$_.FolderPath -join ";"}},ExtendedFolderFlags,MailboxOwnerId | Export-Excel -Path ("$FilePath\$Filename") -WorksheetName "PublicFolder" -AutoSize -AutoFilter -FreezeTopRow -BoldTopRow

##Export Public Folder Client Permission tab
$PublicFolderClientPermission |Select-Object Identity,FolderName,User,@{l='AccessRights';e={$_.AccessRights -join ";"}},SharingPermissionFlags | Export-Excel -Path ("$FilePath\$Filename") -WorksheetName "PublicFolderClientPermission" -AutoSize -AutoFilter -FreezeTopRow -BoldTopRow
